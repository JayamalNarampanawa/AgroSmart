    ...
    <div className={`fixed z-40 ${position === 'bottom-right' ? 'bottom-6 right-6' : 'bottom-6 left-6'}`}>
      <div className="w-80 max-w-xs">
        <div className="flex justify-end">
          <button onClick={()=>setOpen(o=>!o)} className="bg-indigo-600 text-white px-3 py-2 rounded-full shadow">{open ? 'Close' : 'Chat'}</button>
        </div>
        {open && (
          <div className="mt-2 bg-white dark:bg-slate-800 rounded-lg shadow-lg overflow-hidden flex flex-col" style={{height:360}}>
            <div className="px-3 py-2 border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm">AgroBot — Ask about sensors or analytics</div>
            <div ref={listRef} className="flex-1 p-3 overflow-auto space-y-2">
              ...
              {messages.map(m=> (
                <div key={m.id} className={`flex ${m.role === 'assistant' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`${m.role === 'assistant' ? 'bg-slate-100 dark:bg-slate-700 text-slate-900' : 'bg-indigo-600 text-white'} px-3 py-2 rounded-lg max-w-[75%]`}> 
                    <div className="text-xs font-medium mb-1">{m.role === 'assistant' ? (m.name || 'AgroBot') : (m.name || 'You')}</div>
