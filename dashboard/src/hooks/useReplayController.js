...
export default function useReplayController({
  ...
  useEffect(() => {
    if (mode !== 'REPLAY' || !isPlaying || buffer.length === 0) return undefined
  ...
  const enterReplayAtEnd = () => {
    if (buffer.length === 0) return
    setMode('REPLAY')
    setIsPlaying(false)
    setReplayIndex(buffer.length - 1)
  }
  ...
  const atEnd = mode === 'REPLAY' && buffer.length > 0 && replayIndex >= buffer.length - 1
